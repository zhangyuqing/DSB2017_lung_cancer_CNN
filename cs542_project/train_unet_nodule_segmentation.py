...
DATA_DIR = './data_luna16/preprocessed_masks/unet_train/'

EXP_NAME = 'exp_luna16_unet_nodule_segmentation_axis_%d' % SCAN_AXIS
...
scores = unet(image_ph, is_training, kinit, l2reg, NET_NAME)

# Loss
label_ph = tf.placeholder(tf.float32, [None, H_CROP, W_CROP, 1])
...
for n_epoch in range(MAX_EPOCH):
    for n_batch, uid in enumerate(uid_list):
        ...
        d = np.load(os.path.join(DATA_DIR, uid+'_unet_train.npz'))
        ...
        loss_cls_value, _, summary = sess.run((loss_cls, train_step, log_step),
                                              {image_ph: image_crops[..., np.newaxis],
                                               label_ph: label_crops[..., np.newaxis]})
