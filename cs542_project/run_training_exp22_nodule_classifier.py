...
NUM_EPOCHES = 10000
EXP_NAME = 'exp22_nodule_classifier'
...
def model(input_data, kernel_regularizer, is_training, scope='CNN3D', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        output = tf.layers.conv3d(input_data, 32, (4, 4, 4), (1, 1, 1), "VALID", name='conv1',
                                  kernel_initializer=kernel_initializer, kernel_regularizer=kernel_regularizer)
        output = tf.layers.batch_normalization(output, training=is_training, name='batch_norm1')
        output = tf.nn.relu(output, name='relu1')
        output = tf.layers.max_pooling3d(output, (2, 2, 2), (2, 2, 2), "VALID", name='pool1')
        ...
        output = tf.layers.dense(output, 64, name='dense3',
                                 kernel_initializer=kernel_initializer,
                                 kernel_regularizer=kernel_regularizer, activation=tf.nn.relu)
        output = tf.layers.dropout(output, training=is_training, name='drop3')
        output = tf.layers.dense(output, 1, name='dense4',
                                 kernel_initializer=kernel_initializer,
