...
def downsample_to_fixsize(image, new_shape):
    ...
    # Step 1: downsample XY with downscale_local_mean
    downsample_factor = np.array(old_shape) / np.array(new_shape)
    downsample_factor[0] = 1
    image = skimage.transform.downscale_local_mean(image, (1, 8, 8))

    # Step 2: zoom Z with zoom
    image_downsampled = scipy.ndimage.zoom(image, [new_shape[0]/old_shape[0], 1, 1])
    image_downsampled = image_downsampled.astype(np.float32)
...
SAVE_FOLDER = './data/preprocessed_reshaped2/'
SAVE_MEAN_FILE = './data/preprocessed_reshaped2_mean.npy'
