...
print('number of test samples:', num_sample_tst)

# Pre-load all batches into memory
tst_image_array = np.zeros((num_sample_tst, Z, Y, X, 1), np.float32)
...
def model(input_data, weight_regularizer, is_training, scope='CNN3D', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        output = tf.layers.conv3d(input_data, 32, (4, 4, 4), (1, 1, 1), "SAME", name='conv1',
                                  kernel_regularizer=weight_regularizer, activation=tf.nn.relu)
        ...
        output = tf.layers.conv3d(output, 32, (2, 2, 2), (1, 1, 1), "SAME", name='conv3',
                                  kernel_regularizer=weight_regularizer, activation=tf.nn.relu)
        ...
        output = tf.layers.conv3d(output, 32, (2, 2, 2), (1, 1, 1), "SAME", name='conv5',
                                  kernel_regularizer=weight_regularizer, activation=tf.nn.relu)
        ...
        output = tf.layers.conv3d(output, 40, (2, 2, 2), (1, 1, 1), "SAME", name='conv7',
                                  kernel_regularizer=weight_regularizer, activation=tf.nn.relu)
        ...
        output = tf.layers.dense(output, 128, name='dense9', kernel_regularizer=weight_regularizer)
        output = tf.layers.batch_normalization(output, training=is_training, name='batch_norm9')
        output = tf.nn.relu(output, name='relu9')

        output = tf.layers.dense(output, 128, name='dense10', kernel_regularizer=weight_regularizer)
        output = tf.layers.batch_normalization(output, training=is_training, name='batch_norm10')
        output = tf.nn.relu(output, name='relu10')

        output_final = tf.layers.dense(output, 1, name='dense11', kernel_regularizer=weight_regularizer)

    return output_final

# A. Training network
data_trn = tf.placeholder(tf.float32, [None, Z, Y, X, 1], name='data')
output_trn = model(data_trn, weight_regularizer=weight_regularizer, is_training=True)
...
# B. Test network
data_tst = tf.placeholder(tf.float32, [None, Z, Y, X, 1], name='data')
output_tst = model(data_tst, weight_regularizer=None, is_training=False, reuse=True)
...
def run_training():
    ...
    for n_epoch in range(NUM_EPOCHES):
        ...
        for n_batch in range(num_batch):
            ...
            LOADED_DATA = trn_image_array[idx_begin:idx_end]
