...
################################################################################
# The data and params
DATA_DIR = './data/preprocessed_masks/cluster_candidates_with_lidc/'
...
NUM_EPOCHES = 10000
EXP_NAME = 'exp20_nodule_classifier'
...
trn_image_array = np.zeros((num_sample_trn, DATA_C, DATA_Z, DATA_Y, DATA_X, 3), np.float32)
...
for n in tqdm.trange(num_sample_trn):
    ...
    trn_image_array[n, ..., 1:] = d['prob3d_all']
    d.close()
...
# Pre-load all batches into memory
tst_image_array = np.zeros((num_sample_tst, DATA_C, DATA_Z, DATA_Y, DATA_X, 3), np.float32)
...
for n in tqdm.trange(num_sample_tst):
    ...
    tst_image_array[n, ..., 1:] = d['prob3d_all']
    d.close()
...
def model(input_data, kernel_regularizer, is_training, scope='CNN3D', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        output = tf.layers.conv3d(input_data, 16, (4, 4, 4), (1, 1, 1), "VALID", name='conv1',
...
# A. Training network
data_trn = tf.placeholder(tf.float32, [None, DATA_Z, DATA_Y, DATA_X, 3], name='data')
...
# B. Test network
data_tst = tf.placeholder(tf.float32, [None, DATA_Z, DATA_Y, DATA_X, 3], name='data')
...
train_logs = []
train_logs.append(tf.summary.scalar("batch_loss", batch_loss))
log_op_trn = tf.summary.merge(train_logs)

test_logs = []
train_loss_ph = tf.placeholder(tf.float32, [])
train_accuracy_ph = tf.placeholder(tf.float32, [])
test_loss_ph = tf.placeholder(tf.float32, [])
test_accuracy_ph = tf.placeholder(tf.float32, [])
test_logs.append(tf.summary.scalar("train_loss", train_loss_ph))
test_logs.append(tf.summary.scalar("train_accuracy", train_accuracy_ph))
test_logs.append(tf.summary.scalar("test_loss", test_loss_ph))
test_logs.append(tf.summary.scalar("test_accuracy", test_accuracy_ph))
log_op_tst = tf.summary.merge(test_logs)
...
def run_training():
    ...
    for n_epoch in range(NUM_EPOCHES):
        ...
        for n_batch in range(num_batch):
            ...
            LOADED_DATA = np.reshape(trn_image_array[idx_begin:idx_end], [-1, DATA_Z, DATA_Y, DATA_X, 3])
            LOADED_GT = trn_label_array[idx_begin:idx_end]

            # Training step
            loss_value, _, _, summary = sess.run((batch_loss, train_op, update_op, log_op_trn),
        ...
        if (n_epoch+1) % SAVE_EPOCHES == 0:
            ...
            train_loss, train_accuracy = run_test('train')
            test_loss, test_accuracy = run_test('test')
            summary = sess.run(log_op_tst,
                     {train_loss_ph: train_loss,
                      train_accuracy_ph: train_accuracy,
                      test_loss_ph: test_loss,
                      test_accuracy_ph: test_accuracy})
            log_writer.add_summary(summary, n_iter)
...
def run_test(split):
    ...
    for n_batch in range(num_batch):
        idx_begin = n_batch*N
        idx_end = (n_batch+1)*N  # OK if off-the-end
        LOADED_DATA = np.reshape(all_image_array[idx_begin:idx_end], [-1, DATA_Z, DATA_Y, DATA_X, 3])
