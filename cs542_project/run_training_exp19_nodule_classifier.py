...
NUM_EPOCHES = 10000
EXP_NAME = 'exp19_nodule_classifier'
...
def model(input_data, kernel_regularizer, is_training, scope='CNN3D', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        output = tf.layers.conv3d(input_data, 16, (3, 3, 3), (1, 1, 1), "VALID", name='conv1',
                                  kernel_initializer=kernel_initializer,
                                  kernel_regularizer=kernel_regularizer, activation=tf.nn.relu)
        output = tf.layers.conv3d(output, 16, (3, 3, 3), (1, 1, 1), "VALID", name='conv2',
                                  kernel_initializer=kernel_initializer,
                                  kernel_regularizer=kernel_regularizer, activation=tf.nn.relu)
