...
SAVE_DIR = './data/preprocessed_masks/nodule_prob_with_lidc_epoch_%08d/' % SNAPSHOT_EPOCH

TRAINED_MODEL_AXIS_0 = './train/exp_luna16_unet_nodule_segmentation_with_lidc_axis_0/tfmodel/%08d' % SNAPSHOT_EPOCH
TRAINED_MODEL_AXIS_1 = './train/exp_luna16_unet_nodule_segmentation_with_lidc_axis_1/tfmodel/%08d' % SNAPSHOT_EPOCH
TRAINED_MODEL_AXIS_2 = './train/exp_luna16_unet_nodule_segmentation_with_lidc_axis_2/tfmodel/%08d' % SNAPSHOT_EPOCH
...
probs_axis_0 = tf.nn.sigmoid(unet(image_ph, is_training, None, None, 'unet_axiz_0', output_channels=2))
probs_axis_1 = tf.nn.sigmoid(unet(image_ph, is_training, None, None, 'unet_axiz_1', output_channels=2))
probs_axis_2 = tf.nn.sigmoid(unet(image_ph, is_training, None, None, 'unet_axiz_2', output_channels=2))
...
def run_3d_seg_along_axis(image, axis, batch_size=2):
    ...
    prob3d = np.zeros(image.shape + (2,), np.float32)
    ...
    for n_begin in range(0, L, batch_size):
        ...
        if axis == 0:
            ...
            prob2d = sess.run(probs_axis_0, {image_ph: image2d[..., np.newaxis]})
            prob3d[n_begin:n_end, :, :] = prob2d
        elif axis == 1:
            image2d = image[:, n_begin:n_end, :].transpose((1, 0, 2))  # ZYX -> YZX
            prob2d = sess.run(probs_axis_1, {image_ph: image2d[..., np.newaxis]})
            prob3d[:, n_begin:n_end, :] = prob2d.transpose((1, 0, 2, 3))  # YZXC -> ZYXC
        elif axis == 2:
            image2d = image[:, :, n_begin:n_end].transpose((2, 0, 1))  # ZYX -> XZY
            prob2d = sess.run(probs_axis_2, {image_ph: image2d[..., np.newaxis]})
            prob3d[:, :, n_begin:n_end] = prob2d.transpose((1, 2, 0, 3))  # XZYC -> ZYXC
        else:
...
for n in tqdm.trange(len(file_paths)):
    ...
    assert(prob3d.shape[:3] == lung_img.shape)
